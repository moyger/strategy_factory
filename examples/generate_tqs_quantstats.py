"""
Generate QuantStats Report for TQS (Trend Quality Score) Strategy
...
print("="*80)
print("GENERATING QUANTSTATS REPORT - TQS STRATEGY")
print("="*80)
...
output_file = output_dir / f'quantstats_report_tqs_${INITIAL_CAPITAL}.html'
...
qs.reports.html(
    ...
    title=f'Nick Radge TQS Strategy - ${INITIAL_CAPITAL:,} Starting Capital',
    download_filename=f'nick_radge_tqs_report_${INITIAL_CAPITAL}.html'
)
...
# Overall grade
passes = sum([sharpe >= 1.6, sortino >= 2.2, abs(max_dd) <= 22, calmar >= 1.2])
metrics_data['Metric'].append('')
metrics_data['Value'].append('')

metrics_data['Metric'].append('=== OVERALL GRADE ===')
if passes == 4:
    metrics_data['Value'].append('🏆 PROFESSIONAL-GRADE')
elif passes == 3:
    metrics_data['Value'].append('✅ EXCELLENT')
elif passes == 2:
    metrics_data['Value'].append('⚠️ GOOD')
else:
    metrics_data['Value'].append('❌ NEEDS WORK')

# Save metrics
metrics_df = pd.DataFrame(metrics_data)
metrics_file = output_dir / f'metrics_tqs_${INITIAL_CAPITAL}.csv'
...
monthly_file = output_dir / f'monthly_returns_tqs_${INITIAL_CAPITAL}.csv'
...
print(f"   Sharpe Ratio:     {sharpe:.2f} {'✅' if sharpe >= 1.6 else '❌'}")
print(f"   Sortino Ratio:    {sortino:.2f} {'✅' if sortino >= 2.2 else '❌'}")
print(f"   Max Drawdown:     {max_dd:.2f}% {'✅' if abs(max_dd) <= 22 else '❌'}")
print(f"   Calmar Ratio:     {calmar:.2f} {'✅' if calmar >= 1.2 else '❌'}")
print(f"   vs SPY:           {outperformance:+.2f}%")

if passes == 4:
    print(f"\n   🏆 PROFESSIONAL-GRADE STRATEGY!")
elif passes == 3:
    print(f"\n   ✅ EXCELLENT STRATEGY - Very close to professional-grade")
elif passes == 2:
    print(f"\n   ⚠️  GOOD STRATEGY - Room for improvement")
else:
    print(f"\n   ❌ NEEDS WORK - Requires optimization")
