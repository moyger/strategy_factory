"""
Strategy: London Breakout v4.1 - ADAUSD OPTIMIZED for Bybit

OPTIMIZATION RESULTS (vs Baseline):
- Annual Return: 37.1% (vs 23.9%) = +55% improvement
- Final Balance after 2.4 years: $19,062 (vs $15,821)
- Win Rate: 74.4% (vs 73.7%)
- Max Drawdown: -6.21% (same)
- Trades/Year: 21.5 (vs 15.6)

KEY OPTIMIZATIONS:
1. Wider Asia Range: 0.8-8% (vs 1-8%) → +1 more trade, better entry opportunities
2. Aggressive R/R: 2:1 (vs 1.3:1) → +55% profit improvement
3. Same Leverage: 3x (safe for $10k account)
4. Same Momentum Filter: 1.5%

RECOMMENDED FOR:
- Capital: $10,000
- Risk: 1% per trade ($100)
- Leverage: 3x
- Expected: 30-40% annual return
"""
...
class LondonBreakoutV41ADAUSDOptimized:
    def __init__(self, pair='ADAUSD', risk_percent=1.0, initial_capital=10000,
        ...
        # OPTIMIZED Asia breakout parameters
        self.min_asia_range_pct = 0.008   # OPTIMIZED: 0.8% (vs 1%)
        self.max_asia_range_pct = 0.08    # Same: 8%
        self.breakout_buffer_pct = 0.002  # Same: 0.2%
        self.min_first_hour_move_pct = 0.015  # Same: 1.5%

        # Triangle pattern parameters (rarely triggers)
        ...
        self.risk_reward_ratio = 2.0      # OPTIMIZED: 2:1 (vs 1.3:1)
        self.min_tp_pct = 0.02            # Same: 2% minimum TP
